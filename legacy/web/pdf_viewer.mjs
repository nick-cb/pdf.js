...
class PDFPageViewBuffer {
  ...
  cancelRenderingOutsideRange(minId, maxId) {
    for (const view of this.#buf) {
      ...
      if (view.renderingState !== RenderingStates.RUNNING && view.renderingState !== RenderingStates.PAUSED) {
        continue;
      }
      view.cancelRendering({
        abortOperatorList: true
      });
...
class PDFViewer {
  ...
  update() {
    ...
    const preRenderMargin = this._spreadMode !== SpreadMode.NONE && this._scrollMode !== ScrollMode.HORIZONTAL ? 2 : 1;
    this.#buffer.cancelRenderingOutsideRange(visible.first.id - preRenderMargin, visible.last.id + preRenderMargin);
    for (const {
      view,
      visibleArea
    } of visiblePages) {
